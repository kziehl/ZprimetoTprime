...
int main()
{
	...
	TFile *file = new TFile("/nfs/dust/cms/user/kziehl/plots/Zprime/pt_eta/rootfile/BKG_TTbar_true_negativ.root","RECREATE");
	...
	Double_t height_pt=GetMaxVal(chain,"misstagged_top_anti_pt","N_misstagged_top_anti");
	...
	chain->SetBranchStatus("misstagged_top_anti_pt",1);
  chain->SetBranchAddress("misstagged_top_anti_pt",&(pt.front()));
  chain->SetBranchStatus("misstagged_top_anti_eta",1);
  chain->SetBranchAddress("misstagged_top_anti_eta",&(eta.front()));
  ...
  chain->SetBranchStatus("N_misstagged_top_anti",1);
  chain->SetBranchAddress("N_misstagged_top_anti",&N_tagged_top);
  ...
  for(Long64_t i=0; i<nentries; i++)
  {
  	...
  	//save to disk
  	cs->SaveAs("/nfs/dust/cms/user/kziehl/plots/Zprime/pt_eta/pdf/BKG_TTbar_true_negativ.pdf");
  	file->cd();
  	divide->Write("BKG_TTbar_true_negativ");
  	file->Close();
